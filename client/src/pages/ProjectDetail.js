...
const ProjectDetail = () => {
  ...
  const handleTaskEdit = (task) => {
    // ✅ FIXED: Add project context to task
    const taskWithProjectId = {
      ...task,
      projectId: id,
      projectName: project.name
    };
    setEditingTask(taskWithProjectId);
    setShowTaskModal(true);
  };

  // ✅ FIXED: Updated handleTaskUpdate function
  const handleTaskUpdate = (task) => {
    // Ensure the task has projectId and projectName for the modal
    const taskWithProjectId = {
      ...task,
      projectId: id, // Use the project ID from URL params
      projectName: project.name // Add project name for display
    };
    
    console.log('🔄 Opening task update modal for:', taskWithProjectId.title);
    setSelectedTask(taskWithProjectId);
    setShowUpdateModal(true);
  };
